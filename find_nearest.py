...
def save_gif_bbox(file_path, data, heatmap=None, a=.5, b=.5):
    ...
    data = [Image.fromarray(img).quantize(method=Image.MEDIANCUT) for img in data]
...
def find_k_nearest_patches_to_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    for idx, (search_batch_input, search_y,_) in enumerate(dataloader):
        ...
        for img_idx, distance_map in enumerate(proto_dist_):
            for j in range(n_prototypes):
                ...
                if full_save:
                    ...
                    original_img = np.transpose(original_img, (1, 2,3, 0))
    ...
    for j in range(n_prototypes):
        ...
        if full_save:
            ...
            for i, patch in enumerate(heaps[j]):
                ...
                save_gif(os.path.join(dir_for_saving_images,
                                              'nearest-' + str(i+1) + '_original.gif'),
                        patch.original_img)
                ...
                activation_pattern = torch.tensor(patch.act_pattern)
                ...
                upsampled_act_pattern = img_stack_sm
                ...
                act_img = []
                overlayed_patch = []
                for k in range (upsampled_act_pattern.shape[0]):
                    
                    # show the most highly activated patch of the image by this prototype
                    high_act_patch_indices = find_high_activation_crop(upsampled_act_pattern[k,:,:])
                    high_act_patch = patch.original_img[k,high_act_patch_indices[0]:high_act_patch_indices[1],high_act_patch_indices[2]:high_act_patch_indices[3], :]
                    
                    
                    
                    act_img.append(high_act_patch)
                    bbox_img_ = draw_bbox(patch.original_img[k],high_act_patch_indices)
                    bbox_img.append(bbox_img_)
                    # show the image overlayed with prototype activation map
                    rescaled_activation_pattern = upsampled_act_pattern[k] - np.amin(upsampled_act_pattern[k])
                    rescaled_activation_pattern = rescaled_activation_pattern / np.amax(rescaled_activation_pattern)
                    heatmap = cv2.applyColorMap(np.uint8(255*rescaled_activation_pattern), cv2.COLORMAP_JET)
                    heatmap = np.float32(heatmap) / 255
                    heatmap = heatmap[...,::-1]
                    over_img=(0.5 * patch.original_img[k] + 0.3 * heatmap)
                    bbox_img_act_ = draw_bbox(over_img,high_act_patch_indices)
                    bbox_img_act.append(bbox_img_act_)
                    overlayed_patch.append(over_img[high_act_patch_indices[0]:high_act_patch_indices[1],
                                                             high_act_patch_indices[2]:high_act_patch_indices[3], :])
                    overlayed_img.append(over_img)
                save_gif(os.path.join(dir_for_saving_images,
                                               'nearest-' + str(i+1) + '_original_with_heatmap.gif'),
                ...
                # if patch.shape[1] != img_size or patch.shape[2] != img_size:
                    ...
                    #            vmax=1.0)
                    # save_gif(os.path.join(dir_for_saving_images,
                    #                      'nearest-' + str(i+1) + '_receptive_field.gif'),
                    #         np.asarray(patch.patch))
                    ...
                    #            vmax=1.0)
                save_gif(os.path.join(dir_for_saving_images,
                                              'nearest-' + str(i+1) + '_receptive_field_with_heatmap.gif'),
                            np.asarray(overlayed_patch))
                ...
                #            vmax=1.0)
                save_gif(os.path.join(dir_for_saving_images,
                                              'nearest-' + str(i+1) + '_high_act_patch.gif'),
                            np.asarray(act_img))
                
                # save the original image with bounding box showing high activation patch
                save_gif_bbox(os.path.join(dir_for_saving_images,
                                       'nearest-' + str(i+1) + '_high_act_patch_in_original_img.gif'),
                            np.asarray(bbox_img_act))
