...
def pair_wise_cos(x,y,c=1.0):
    ...
    denominator = torch.matmul(x_norm.cuda(), y_norm.t().cuda())  # m x n
    numerator = torch.matmul(x.cuda(), y.t().cuda())  # m x n, each element is a in-prod
...
#             if i != j:
#                 loss = loss+ torch.max(torch.Tensor([0]).expand_as(x1[i]),F.cosine_similarity(x1[i],x2[j]))
                
#                 # loss_.append(loss)
#     # return torch.sum(torch.Tensor(loss_))
#     return torch.mean(loss)

# x1 = torch.randn(1010,64,1,1,1)
# x2 = torch.randn(1010,64,1,1,1)
# print(diversity_loss(x1,x2))
...
#         # Step1: Calculate the l2-norm of prototypes 
#         norm = prototypes.norm(p = 2,dim = 1)

#         norm_col = norm.reshape([-1, 1])
#         norm_row = norm.reshape([1, -1])
        
#         # Step2: Calculate cos using definition cos(pi, pj) = <pi,pj> / ||pi||*||pj||
#         numerator = (prototypes @ prototypes.T)
#         denominator = norm_col @ norm_row

#         pairwise_cos = numerator / denominator

#         # Step3: Removing the effect of self-cosine.
#         diag_one = torch.diag(torch.ones(n)).cuda()
#         pairwise_cos_off_diag = pairwise_cos - diag_one

#         # Step4: Removing the effect of negative negative cosines (optional)
#         pairwise_cos_violation = pairwise_cos_off_diag - self.s_max
#         pairwise_cos_violation[pairwise_cos_violation < 0] = 0
        
#         return pairwise_cos_violation.max()
    
# def validation(prototypes_input):
    
#     prototypes = prototypes_input.detach()
...
#     # Step4: Removing the effect of negative negative cosines (optional)
#     pairwise_cos_off_diag[pairwise_cos_off_diag < 0] = 0
    
#     print(f'Max cos: {pairwise_cos_off_diag.max()}, Avg cos: {pairwise_cos_off_diag.mean()}')
    
#     return pairwise_cos_off_diag.max()
    
# n, d, l,w,t = 1010, 64, 1,1,1
# s_max = 0.1
# prototypes = Parameter(torch.randn((n, d, l,w,t),requires_grad=True, device="cuda"))
# prototypes = prototypes.reshape(n,d)
# optimizer = torch.optim.Adam([prototypes], lr=1e-3)

# model = Model(s_max).cuda()

# time_list , max_cos_curve = [], []

# for t in range(10000):

#     loss = model(prototypes)

#     if t % 200 == 199:
#         print(t, loss.item())
#         max_cos = validation(prototypes).item()
        
#         time_list.append(t)
#         max_cos_curve.append(max_cos)
