...
def main_worker(index, opt):
    ...
    label_set = label_set_()
    emb = get_emb('hyp', opt.emb_file, label_set)
    son2parent,_,_ ,_,_,_= get_son2parent(opt.tree_file)
    ...
    if not opt.no_val:
        test_loader, test_logger ,label_set = get_val_utils(opt)
    ...
    save_analysis_path = os.path.join(opt.result_path,"trial")
                ...
                if batch_idx == 9: 
                    ...
                    for img_idx in range (images_test.shape[0]):
                        if img_idx <1:
                            ...
                            makedir(os.path.join(save_analysis_path, 'most_activated_prototypes'+str(batch_idx)+str(img_idx)))

                            log('Most activated 10 prototypes of this image:')
                            print(torch.sort(prototype_activations[img_idx])[-5:])
                            array_act, sorted_indices_act = torch.sort(prototype_activations[img_idx])
                            for proto_i in range(1,11):

                                log('top {0} activated prototype for this image:'.format(proto_i))
                                save_prototype(os.path.join(save_analysis_path, 'most_activated_prototypes'+str(batch_idx)+str(img_idx),
                                                            'top-%d_activated_prototype.gif' % proto_i),
                                            start_epoch_number, sorted_indices_act[-proto_i].item())

                                save_prototype_self_activation(os.path.join(save_analysis_path, 'most_activated_prototypes'+str(batch_idx)+str(img_idx),
                                                                            'top-%d_activated_prototype_self_act.gif' % proto_i),
                                                            start_epoch_number, sorted_indices_act[-proto_i].item())
                                
                                log('prototype index: {0}'.format(sorted_indices_act[-proto_i].item()))
                                log('activation value (similarity score): {0}'.format(array_act[-proto_i]))
                                log('last layer connection with predicted class: {0}'.format(model.module.last_layer.weight[predicted_cls][sorted_indices_act[-proto_i].item()]))
                                
                                activation_pattern = prototype_activation_patterns[img_idx][sorted_indices_act[-proto_i].item()].detach().cpu().numpy()
                                ...
                                for k in range (upsampled_activation_pattern.shape[0]):
                                    ...
                                    high_act_patch = original_img[k,high_act_patch_indices[0]:high_act_patch_indices[1],high_act_patch_indices[2]:high_act_patch_indices[3], :]
                                ...
                                save_gif(os.path.join(save_analysis_path, 'most_activated_prototypes'+str(batch_idx)+str(img_idx),
                                                        'most_highly_activated_patch_by_top-%d_prototype.gif' % proto_i),
                                        np.asarray(act_img))
                                log('most highly activated patch by this prototype shown in the original image:')
                                
                                save_gif_bbox(os.path.join(save_analysis_path, 'most_activated_prototypes'+str(batch_idx)+str(img_idx),
                                                        'most_highly_activated_patch_in_original_img_by_top-%d_prototype.gif' % proto_i),np.asarray(bbox_img))
                                log('prototype activation map of the chosen image:')
                                
                                save_gif(os.path.join(save_analysis_path, 'most_activated_prototypes'+str(batch_idx)+str(img_idx),
                                                        'prototype_activation_map_by_top-%d_prototype.gif' % proto_i),
