...
def collate_fn(batch):
    batch_clips, batch_targets, batch_emb = zip(*batch)
    ...
    batch_emb = [
        emb for multi_emb in batch_emb for emb in multi_emb
    ]

    target_element = batch_targets[0]
    if isinstance(target_element, int) or isinstance(target_element, str):
        return default_collate(batch_clips), default_collate(batch_targets), default_collate(batch_emb)
    else:
        return default_collate(batch_clips), batch_targets,batch_emb
...
class VideoDatasetMultiClips(VideoDataset):
    ...
    def __getitem__(self, index):
        ...
        if 'segment' in self.target_type:            
            ...
            a_batches = [self.emb[i,:] for i in (targets)]        
            return clips, targets, a_batches