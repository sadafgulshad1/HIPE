...
from dataset_hyperbolic import get_emb
from utils import Logger
from utils import label_set_
label_set = label_set_()
tree_file = "/var/scratch/sgulshad/Postdoc/Hierarchical_ProtoPNet_Video/Code_from_system/ResNet_3D_Original_Code/3D-ResNets-PyTorch_Hyperbolic/UCF-101_hierarcy.csv"
from dataset_hyperbolic import get_son2parent
son2parent, _, _, _ , _, _ = get_son2parent(tree_file)
...
def evaluate(ground_truth_path, result_path, subset, top_k, ignore):
    ...
    res = [l[0] for l in result.values()]
    ground = [l[1] for l in ground_truth]
    
    hop1_acc = metric.hop_acc(res,ground, hops =2)
    hop2_acc = metric.hop_acc(res,ground, hops =4)
    print ("hop1_acc is ",hop1_acc)
    print ("hop2_acc is ",hop2_acc)
    return accuracy, hop1_acc, hop2_acc
...
if __name__ == '__main__':
    ...
    accuracy, hop1_acc, hop2_acc = evaluate(args.ground_truth_path, args.result_path, args.subset,
                        args.k, args.ignore)

    if args.save:
        
        with (args.result_path/'top{}.txt'.format(
                args.k)).open('w') as f:
            f.write(str(accuracy))
            f.write(str(hop1_acc))
            f.write (str(hop2_acc))
